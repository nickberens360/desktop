<template>
  <div class="file-window-container absolute">
    <DragBox class="file-window__drag-box" :id="id" :handle="$refs[`handle-${id}`]" :initial-z-index="initialZIndex">
      <template #default>
        <div
          :id="id"
          :ref="id"
        ...
        >
          <div :id="`handle-${id}`" :ref="`handle-${id}`"  class="px-4 py-2 cursor-grab flex items-center justify-between" style="border-bottom: 1px solid #414141;">
              <div>
                <button
